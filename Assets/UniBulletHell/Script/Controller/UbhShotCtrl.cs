...
public sealed class UbhShotCtrl : UbhMonoBehaviour
{
    ...
    public float m_startOnAwakeDelayMin = 1f;
    public float m_startOnAwakeDelayMax = 1f;
    ...
    private void Start()
    {
        ...
        if (m_startOnAwake)
        {
            StartShotRoutine(UnityEngine.Random.Range(m_startOnAwakeDelayMin, m_startOnAwakeDelayMax));
        }
    }
    ...
    {
        if (m_shooting == false || GameController.Instance.currentState == State.Paused || GameController.Instance.currentState == State.Initial || GameController.Instance.currentState == State.LevelUp || GameController.Instance.currentState == State.Death || GameController.Instance.currentState == State.Dialog)
        {
            return;
        }
